...
public class RMAppImpl implements RMApp, Recoverable {
	...
	private static final ApplicationResourceUsageReport DUMMY_APPLICATION_RESOURCE_USAGE_REPORT = BuilderUtils
	...
	public RMAppImpl(ApplicationId applicationId, RMContext rmContext,
		...
		try {
			...
			transitionLogger = new BufferedWriter(new FileWriter(
					transitionLogFile, true));
		...
		if (individualMaxAppAttempts <= 0
			...
			LOG.warn("The specific max attempts: " + individualMaxAppAttempts
					...
					+ "]. Use the global max attempts instead.");
		} else {
	...
	public FinalApplicationStatus getFinalApplicationStatus() {
		this.readLock.lock();
		try {
			// finish state is obtained based on the state machine's current
			// state
			...
			return createFinalApplicationStatus(this.stateMachine
					.getCurrentState());
		} finally {
	...
	public ApplicationReport createAndGetApplicationReport(boolean allowAccess) {
		...
		try {
			...
			if (allowAccess) {
				if (this.currentAttempt != null) {
					currentApplicationAttemptId = this.currentAttempt
							.getAppAttemptId();
					trackingUrl = this.currentAttempt.getTrackingUrl();
					origTrackingUrl = this.currentAttempt
							.getOriginalTrackingUrl();
					Token<ClientToAMTokenIdentifier> attemptClientToAMToken = this.currentAttempt
							.getClientToAMToken();
						...
						if (getApplicationSubmissionContext().getUnmanagedAM()
								&& getUser().equals(
										UserGroupInformation.getCurrentUser()
												.getUserName())) {
							Token<AMRMTokenIdentifier> token = currentAttempt
									.getAMRMToken();
							if (token != null) {
								amrmToken = BuilderUtils.newAMRMToken(token
										.getIdentifier(), token.getKind()
										.toString(), token.getPassword(), token
										.getService().toString());
							}
						}
					} catch (IOException ex) {
						LOG.warn(
								"UserGroupInformation.getCurrentUser() error: "
										+ ex.toString(), ex);
					}
				}
			}
			...
			return BuilderUtils
					.newApplicationReport(this.applicationId,
			...
			if (oldState != getState()) {
				...
				try {
					transitionLogger.append("Sakshi " + appID
							...
							+ getState() + "after EVENT: " + event.getType()
							+ "\n");
	...
	private void createNewAttempt(boolean startAttempt) {
		ApplicationAttemptId appAttemptId = ApplicationAttemptId.newInstance(
				applicationId, attempts.size() + 1);
		RMAppAttempt attempt = new RMAppAttemptImpl(appAttemptId, rmContext,
				scheduler, masterService, submissionContext, conf, user);
		attempts.put(appAttemptId, attempt);
		currentAttempt = attempt;
		if (startAttempt) {
			handler.handle(new RMAppAttemptEvent(appAttemptId,
	...
	private static final class RMAppSavingTransition extends RMAppTransition {
		@Override
		public void transition(RMAppImpl app, RMAppEvent event) {
			// If recovery is enabled then store the application information in
			// a
	...
	private static class FinalTransition extends RMAppTransition {
		...
		public void transition(RMAppImpl app, RMAppEvent event) {
			...
			for (NodeId nodeId : nodes) {
				app.handler.handle(new RMNodeCleanAppEvent(nodeId,
						app.applicationId));
			}
		...
		public RMAppState transition(RMAppImpl app, RMAppEvent event) {
			...
			if (app.submissionContext.getUnmanagedAM()) {
				...
				msg = "Application " + app.getApplicationId() + " failed "
						+ app.maxAppAttempts + " times due to "
